...
| **Pattern (Regex)** | Deterministic pattern matching over the text | No LLM | `ssn_matches`, employee IDs |
| **Rule-based** | Deterministic conditions over *other tags'* values | No LLM | `AI Agent Access = Restricted` if `PII = true` |

Rule-based tags evaluate after their input tags exist: if a rule matches, the value is resolved deterministically; if no rule matches, the tag falls back to LLM classification. Because rules run at zero LLM cost, they are the right tool for policy verdicts derived from other signals (access gates, retention classes, readiness flags).
...
# If a rule matches, the value resolves with no LLM call;
# if none match, the tag falls back to LLM classification.
client.tags.upsert(tag_data={
