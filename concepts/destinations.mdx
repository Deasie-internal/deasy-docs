...
```python
# Export enriched data to destination
...
if result.tracker_id:
    status = client.progress_tracker.get_status(
...
for d in destinations.destinations:
    print(f"{d.destination_name}: {d.vector_db_type}")
```
  </Tab>
  <Tab title="Delete Destination">
```python
# Delete a destination
client.destination.delete(destination_name="my-vector-db")
