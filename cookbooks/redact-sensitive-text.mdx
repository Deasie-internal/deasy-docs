...
flowchart LR
    ...
    TXT[Document text<br/>data_source.document_text] --> MASK[Custom Python<br/>str.replace]
    VAL --> MASK
...
```python
text_response = client.data_source.document_text(
    data_connector_name=CONNECTOR,
    file_names=[file_name],
)

safe_nodes = {
    node_id: redact(text, matches_by_file.get(file_name, []))
    for node_id, text in text_response.file_to_nodes_to_text[file_name].items()
}

response = your_llm_client.complete(prompt="\n".join(safe_nodes.values()))
```
