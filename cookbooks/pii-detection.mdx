...
<Note>
You do not write detection logic. The classifiers are built in, and when sensitivity detection runs, the platform automatically adds three rule-based rollup tags: `PII (Personally Identifiable Information)`, `PCI (Payment Card Industry Data)`, and `PHI (Protected Health Information)`. PII detection currently supports English documents only.
</Note>
...
flowchart LR
    ...
    RUN --> ROLL[Rollup rule tags<br/>PII · PCI · PHI]
...
Include the sensitivity tags in a classification job. The platform runs its sensitivity strategy at the highest merge priority and writes the detected values, plus the `PII`, `PCI`, and `PHI` rollup tags, as file-level metadata.
